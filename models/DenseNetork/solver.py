...
class Solver(object):
    ...
    def __init__(self, input_dir, output_dir, model, device, per_gpu_batch_size, n_gpu, batch_size, learning_rate,
        ...
        logx.initialize(logdir=output_dir,
                        ...
                        tensorboard=True,
                        no_timestamp=False,
                        hparams={"solver_hparams": self.state_dict(), "model_hparams": model.hparams_dict()},
                        eager_flush=True)
        # arguments
    ...
    def from_pretrained(cls, model_constructor, pretrained_system_name_or_path, resume_training=False,
        ...
        model = model_constructor.from_pretrained(meta[model_key])  #
        ...
        with tqdm(total=len(self.train_dataloader), desc=f"Epoch {epoch_idx}") as pbar:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if (batch_idx + 1) % steps_per_eval == 0:
                    ...
                    if self.n_gpu > 1:
                        save_dict = {"model_construct_params_dict": MODEL_TYPE,
    ...
    def batch_to_device(self, batch):
        ...
        batch_input = {
            ...
            'attention_mask': attention_mask.to(torch.int64).to(self.device),
